...
async function openWithAI(prompt) {
  ...
  let provider = new AIProvider(prompt, url);
  await provider.open();
}
...
class AIProvider {
  ...
  async open() {
    try {
      let tab = await this.createTab(this.url);
      console.log(tab);
      // tab should be fully loaded before sending the message
  ...
  // Helper function to wait until the tab is fully loaded
  async waitForTabToLoad(tabId) {
